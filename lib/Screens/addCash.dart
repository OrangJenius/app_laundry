...
class AddCashScreen extends StatefulWidget {
  ...
  const AddCashScreen({super.key, required this.storeId});
...
class _AddCashScreenState extends State<AddCashScreen> {
  ...
  Future<void> _submitAddCash() async {
    ...
    if (jumlahText.isEmpty || (double.tryParse(jumlahText) ?? 0) <= 0) {
    ...
    try {
      await _cashFlowService.addModal(
        jumlah: jumlahText,
      ...
      if (!mounted) return;
      _showSnackBar("Kas berhasil ditambahkan");
      Navigator.pop(context, true); // signal caller to refresh
    } catch (e) {
      if (!mounted) return;
      _showSnackBar("Gagal menambahkan kas: $e");
    } finally {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            children: [
              UpperBar2(title: "PENAMBAHAN CASH"),
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _isSubmitting ? null : _submitAddCash,
